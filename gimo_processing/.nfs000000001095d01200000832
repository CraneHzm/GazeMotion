...
dataset_processed_path = "/scratch/hu/pose_forecast/gimo_gazemotion/"
remake_dir(dataset_processed_path)
remake_dir(dataset_processed_path + "train/")
remake_dir(dataset_processed_path + "test/")
dataset_info = pd.read_csv('dataset.csv')
training_item_num = 0
training_data_num = 0
test_item_num = 0
test_data_num = 0
check_gaze_head_distance = False
...
for i, seq in enumerate(dataset_info['sequence_path']):
    ...
    for j in range(data_num):
        ...
        if os.path.exists(pose_path):
            ...
            pose_ori_local = Rotation.identity().as_rotvec()
            pose_ori_local = torch.from_numpy(pose_ori_local.copy()).float()
            #print(pose_ori_local)            
    ...
    if dataset_info['training'][i] == 1:
        gaze_file = dataset_processed_path + "train/" + scene + "_" + seq + "_" + str(start_frame) + "_" +"gaze.npy"
        pose_file = dataset_processed_path + "train/" + scene + "_" + seq + "_" + str(start_frame) + "_" +"pose_xyz.npy"
        head_file = dataset_processed_path + "train/" + scene + "_" + seq + "_" + str(start_frame) + "_" +"head.npy"
    ...
    if dataset_info['training'][i] == 0:
        gaze_file = dataset_processed_path + "test/" + scene + "_" + seq + "_" + str(start_frame) + "_" + "gaze.npy"
        pose_file = dataset_processed_path + "test/" + scene + "_" + seq + "_" + str(start_frame) + "_" +"pose_xyz.npy"
        head_file = dataset_processed_path + "test/" + scene + "_" + seq + "_" + str(start_frame) + "_" +"head.npy"
