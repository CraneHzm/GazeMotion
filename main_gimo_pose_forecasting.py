...
from model import gaze_forecasting, pose_forecasting, pose_forecasting2
...
def main(opt):        
    ...
    print('>>> create model')
    if opt.use_gaze == 1:
        # eye gaze data in gimo dataset is noisy, making it hard to predict future eye gaze
        # therefore, we choose to directly use the raw eye gaze as input
        print("use raw eye gaze direction")
    ...
    if opt.use_gaze == 3:
        # 'pose_forecasting2' performs gaze/head forecasting/preprocessing + pose forecasting in an end-to-end way
        print("use raw head direction")
        net_pred = pose_forecasting2.pose_forecasting(opt=opt).to(opt.cuda_idx)
    ...
    train_dataset = gimo_dataset.gimo_dataset(data_dir, input_n, output_n, train_flag = 1, sample_rate = opt.train_sample_rate)
    ...
    valid_dataset = gimo_dataset.gimo_dataset(data_dir, input_n, output_n, train_flag = 0)
    ...
    for epo in range(start_epoch, opt.epoch + 1):
        ...
        ret_train = run_model(net_pred, optimizer, is_train=1, data_loader=train_loader, opt=opt)
        ...
        if epo % opt.validation_epoch == 0:
            ...
            ret_valid = run_model(net_pred, is_train=0, data_loader=valid_loader, opt=opt)                        
...
def eval(opt):        
    ...
    test_dataset = gimo_dataset.gimo_dataset(data_dir, input_n, output_n, train_flag = 0)
...
def run_model(net_pred, optimizer=None, is_train=1, data_loader=None, opt=None):
    ...
    for i, (data) in enumerate(data_loader):
        ...
        if opt.use_gaze == 1:
            #gaze_input = gaze_input[:, -1:, :].expand(-1, input_n, -1).clone()
