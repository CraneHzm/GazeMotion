...
class adt_dataset(Dataset):
    ...
    def load_data(self, data_dir, input_n, output_n):
        ...
        for i in range(segments_number):
            ...
            data_size = seq_sel.shape[0]
            seq_sel = seq_sel[::self.sample_rate, :, :]
            #print(seq_sel.shape)
...
if __name__ == "__main__":
    data_dir = "/scratch/hu/pose_forecast/adt_gazemotion/"
    input_n = 10
    output_n = 30
    train_dataset = adt_dataset(data_dir, input_n, output_n, train_flag = 1, sample_rate=2)
    print("Training data size: {}".format(train_dataset.pose_gaze_head.shape))
    test_dataset = adt_dataset(data_dir, input_n, output_n, train_flag = 0)
